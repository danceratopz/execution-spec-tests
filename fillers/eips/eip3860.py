...
class TestContractCreationGasUsage:
    ...
    def test_gas_usage(
        ...
        limits.
        """
        if (gas_test_case == "too_little_execution_gas") and (exact_execution_gas == exact_intrinsic_gas):
            pytest.skip(
...
class TestCreateInitcode:
    ...
    def test_create_opcode_initcode(
        ...
        expected_gas_usage = (
            CREATE_CONTRACT_BASE_GAS + GAS_OPCODE_GAS + (2 * PUSH_DUP_OPCODE_GAS) + CALLDATASIZE_OPCODE_GAS
        )
        if opcode == Op.CREATE2:
