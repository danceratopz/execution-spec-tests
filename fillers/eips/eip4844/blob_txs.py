...
def total_account_minimum_balance(  # noqa: D103
    ...
    for tx_blob_count in [len(x) for x in blob_hashes_per_tx]:
        ...
        total_cost += (tx_gas * (block_fee_per_gas + tx_max_priority_fee_per_gas)) + tx_value + data_cost
    return total_cost
...
def all_valid_blob_combinations() -> List[Tuple[int, ...]]:
    ...
    all = [
        seq
        for i in range(MAX_BLOBS_PER_BLOCK, 0, -1)  # We can have from 1 to at most MAX_BLOBS_PER_BLOCK blobs per block
